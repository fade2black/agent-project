...
use crate::Winners;
use crate::{CbbaGossip, Winner};
use crate::{Location, Task, TaskStore};
use bytes::BytesMut;
...
pub struct CbbaPhase<T: Task> {
    port: u16,
    agent_id: u32,
    timeout: u64,
    bundle: Bundle,
    winners: Winners,
    tasks_store: TaskStore<T>,
}
...
where
    T: Task,
{
    pub fn new(agent_id: u32, port: u16, timeout: u64) -> Self {
        Self {
            port,
            agent_id,
            timeout,
            bundle: Bundle::new(),
            winners: Winners::new(),
            tasks_store: TaskStore::new(),
        }
    }

    pub async fn run_cbba_phase(&mut self) -> Result<(), CbbaError> {
        let mut bytes = BytesMut::new();

        self.init();
        let mut sender = UdpTransport::new_sender(self.port).await?;
        let mut receiver = UdpTransport::new_receiver(self.port).await?;

        let timer = time::sleep(Duration::from_secs(self.timeout));
        tokio::pin!(timer);

        info!("⚙️ Starting CBBA phase...");

        //Send initial gossip
        self.send_gossip(&mut sender).await;
    ...
    fn process_gossip(&mut self, gossip: &CbbaGossip) -> bool {
    ...
    fn compare(&self, remote: &Winner, local: &Winner) -> ConflictDecision {
    ...
    fn init(&mut self) {
        let ctx = self.create_task_context();

        // Compute initial bids and initialize the bundle and winners
        let bids = self.tasks_store.compute_local_bids(&ctx);
        self.bundle.init(bids.keys().cloned().collect());
        self.winners.init(1000, bids);
    }

    fn create_task_context(&self) -> TaskContext {
        TaskContext {
            task_count_weight: 0.75,
            task_count: self.tasks_store.tasks_count(),
            agent_location: Location { lat: 0.0, lon: 0.0 },
            energy: 100.0,
        }
    }

    async fn send_gossip(&mut self, sender: &mut UdpTransport) {
